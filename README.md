...
The unified `fit_sarimax()` (`sarimax/src/fit.r` calls it via `window_for_train_id()`, which resolves `train_1`..`train_4` to their `train_start`/`train_end`/`forecast_start`/`forecast_end` bounds; `train_4` in particular resolves to the `SUBMISSION_WINDOW` set at the top of `fit.r` -- see the `if (train_id == "train_4")` .../`window_for_train_id()` logic in the dengue and chikungunya prediction loops near the bottom of `fit.r`) is the function used for every one of the four validation splits, not just the submission window. Two mechanisms keep every split leak-free with respect to its own cutoff:
